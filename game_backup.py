...
def markGuessHard(word, guess, alphabet):
    g = guess.getWord()
    w = word 
    a = alphabet.getWord()
...
def playRound(player, words, all_words, settings, hardmode):
    if hardmode == False:
        ...
        while i < 6:
            ...
            if len(player_guess) != 5 or not all_words.contains(player_guess):  #checks if word doesn't have 5 letters or not in file of words
            ...
            if player_guess == ranWord: #goes here if players's guess is green
                ...
                playWordle() #in order to exit out of function
            else:
...
def playWordle():
    ...
    if ready_to_play == "y":
        ...
        print("Type the letter h if you would like to get a hint for a letter, but be careful. It takes away one of your attempts")
    ...
    if play_again == "y":
        hardmode = input("Would you like to activate hard mode?(y/n) ")
        if hardmode == "y":
            print("Are you sure? You cannot go back to normal mode ")
            print("Be careful what you wish for...")
            playRound(player_list, five_letter_words, all_words, settings, True)

    
    if play_again.lower() == "n":  #goes here if player plays only one round
        player.displayStats()


    if play_again.lower() == "y": #while loop in order to check if player wants to play after numerous rounds have been playerd     
            playRound(player_list, five_letter_words, all_words, settings, False)
